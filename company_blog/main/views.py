...
def create_post():
    form = BlogPostForm()
    if form.validate_on_submit():
        blog_post=BlogPost(title=form.title.data, text=form.text.data, featured_image=form.picture.data, user_id=current_user.id, category_id=form.category.data, summary=form.summary.data)
...
def blog_maintenance():
    page=request.args.get('page', 1, type=int)
    blog_post=BlogPost.query.order_by(BlogPost.id.desc()).paginate(page=page, per_page=10)
    return render_template('blog_maintenance.html', blog_posts=blog_post)
